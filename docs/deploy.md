...
[`migrations/0002_add_reward_art_keys.sql`](../migrations/0002_add_reward_art_keys.sql) は `tracks` に nullable な `art_key` と `reward_key` を追加する追加専用 migration である。本アドオンをデプロイする際は、対象データベースを確認したうえで次のコマンドを再実行し、未適用の 0002 を本番へ適用する。
...
`build-segments.mjs` の完了後、R2 へのアップロード前に `extract-art.mjs` を実行する。埋め込みアートがあるトラックだけを抽出して `.audio/manifest.json` に `artKey` をマージし、アートがないトラックはスキップされる。詳細なコマンドは [`scripts/README.md`](../scripts/README.md) を参照する。

本番に既に投入済みのトラックへ `art_key` や将来の `reward_key` を後付けする場合は、マニフェストへキーを追加し、`gen-seed.mjs --mode update` で UPDATE のみを生成してから本番 D1 に適用する。リワードクリップは現時点では手動で R2 に投入し、そのキーを同じ手順で紐付ける。

### 3. R2 にセグメントとアートをアップロードする
...
このスクリプトは各セグメントに `audio/mp4`、各アート画像に `image/jpeg` を指定して `wrangler r2 object put ... --remote` を実行する。本番 R2 を変更するため、対象バケットを確認してから実行する。アップロード前の確認には `--dry-run` を使う。

### 4. D1 用 seed を生成・適用する
...
既存行のアート/リワードだけを更新する場合は、`node scripts/gen-seed.mjs --manifest .audio/manifest.json --output reward-art-update.sql --mode update` を使う。詳細な手順と、コーラスのみの `.m4a` リワードクリップを手動生成して `audio/mp4` で R2 に投入する方法は [`scripts/README.md`](../scripts/README.md) にまとめている。

### 5. パイプラインの構文を確認する

```sh
node --check scripts/build-segments.mjs
node --check scripts/extract-art.mjs
...
- `GET /api/game/:sid/q/:n/reward` と `GET /api/game/:sid/q/:n/art` は、そのセッションのその問題が正解済みで、対応する `reward_key`/`art_key` が非 NULL のときだけ 200 を返す。それ以外は理由を問わず 404 である。リワードクリップは本アドオン時点では全トラック未投入なので、`/reward` が常に 404 でも正常である。
